function run_comparisonRGB_PSNR(conf)
% The results are written into HTML report, together with thumbnails

fid = fopen(fullfile(conf.result_dirRGB, 'PSNR.html'), 'wt');
fprintf(fid, ...
...
for i = 1:numel(conf.filenames)        
    ...
    for j = 2:numel(conf.results{i})
        ...
        scores(i,j) = score;
        fprintf(fid, '<TD><A HREF=%s>%.6f</A>(%.2f)</TD>\n', ...
...
for i = 2:length(mscores)
    fprintf(fid, '<TD>%.6f (%.2f)</TD>\n', mscores(i), mean(conf.countedtime(i-1,:)));
    fprintf(' : %.6f dB', mscores(i));    
end;
fprintf(fid, '</TR>\n');
fprintf('\n');
fprintf(fid, '</TABLE>\n');
