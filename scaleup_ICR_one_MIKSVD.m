function [imgs, midres] = scaleup_ICR_one_MIKSVD(conf, imgs, LR)

% Super-Resolution Iteration
    fprintf('Scale-Up ICR');
    midres = resize(LR, conf.upsample_factor, conf.interpolate_kernel);    
    %midres = imgs;
%     upsample_factor_temp = conf.upsample_factor;
%     conf.upsample_factor = 1;
    for i = 1:numel(midres)
        features = collect(conf, {imgs{i}}, conf.upsample_factor, conf.filters);
        ...
        if size(conf.dict_lores_MIKSVD,2) > 10000
            blocksize = 500;
        end
        if size(features,2) < blocksize
            D = abs(conf.dict_lores_MIKSVD'*features); 
...
%             end
            ...
            for b = 1:blocksize:size(features,2)
                if b+blocksize-1 > size(features,2)
                    D = abs(conf.dict_lores_MIKSVD'*features(:,b:end));
                    %D = conf.pointslo'*features(:,b:end);
                else
                    D = abs(conf.dict_lores_MIKSVD'*features(:,b:b+blocksize-1));                 
...
%     conf.upsample_factor = upsample_factor_temp;
fprintf('\n');
