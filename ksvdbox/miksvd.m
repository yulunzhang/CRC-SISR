function [D,Gamma,err,gerr] = miksvd(params,varargin)
...
for iter = 1:iternum
  ...
  for j = 1:dictsize
    [D(:,p(j)),gamma_j,data_indices,unused_sigs,replaced_atoms] = optimize_atom_bo(data,D,p(j),Gamma,unused_sigs,replaced_atoms, params);
...
function [atom,gamma_j,data_indices,unused_sigs,replaced_atoms] = optimize_atom_bo(X,D,j,Gamma,unused_sigs,replaced_atoms, params)
...
%   tradeoff = 0.05;
  ...
  for t = 1:maxit
      atomdelta = 0;
      atomcorr = atom'*D;
      for s = 1:length(atomcorr)
          if s~=j
             atomdelta = atomdelta + sign(atomcorr(s))*D(:,s);
          end
      end
      atomdelta = tradeoff*atomdelta/(length(atomcorr)-1);
      atom = (atom - atomdelta);
      atom = atom/norm(atom);
  end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Liefeng Bo's code
